	...
	<div>
		...
		<div class="payinf">
			...
			<div>
				...
				<div><span>预约上门</span><span>￥：{{serverinf.makePrice}}</span></div>
			</div>
		</div>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			this.serverinf = wx.getStorageSync('custominf');
			...
			req_unifiedOrder() {
				...
				ut.request({
					data: {
						...
						"remark": this.remark,
						"goodsId": this.serverinf.id
					},
					url: "customze/store/order/unifiedOrder",
					c: true
				}).then(data => {
					wx.requestPayment({
