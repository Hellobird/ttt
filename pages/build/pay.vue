<template>
	<div>
		...
		<div class="payinf inff" v-for="list in buildinf" :key='list'>
			<div>
				<image :src="static+list.picture" class="goods_image"></image>
				<div>
					<p>
		...
		<div class="payinf time">
			...
			<p>
				<view>
					<picker mode="date" value="日期" :custom-item="customItem" :start="start" :end="end" class='regionpicker' v-if="!disabled"
					 @change="dateChange">
						<view class="picker">
							日期
						</view>
				...
				<view>
					<picker mode="time" :value="startTime" :custom-item="customItem" :start="startTime" end="23:59" class='regionpicker'
					 v-if="!disabled" @change="timeChange">
						<view class="picker">
							时间
						</view>
					</picker>
		...
		<div class="payinf sendtype">
			<div class="payinf_title">
				<h1>配送方式</h1>
				<view class="intro_right" @click="clickStatement(3)">
					<image src="../../static/right_flag.png" class="intro_image"/>
					<view>配送声明</view>
				</view>
			</div>
		...
		<div class="payinf sendtype sendtype1">
			<view class="payinf_title">
				<h1>是否搬运上楼</h1>
				<view class="intro_right" @click="clickStatement(4)">
					<image src="../../static/right_flag.png" class="intro_image"/>
					<view>搬运声明</view>
				</view>
			</view>
			<p class="carry">
				<image src="../../static/mine/uncheck.jpg" v-if="requireCarry==1"></image>
				<image src="../../static/mine/check.jpg" v-else @click="cg_requireCarry(1)"></image><span>是</span>
				<image src="../../static/mine/uncheck.jpg" v-if="requireCarry==2"></image>
				<image src="../../static/mine/check.jpg" v-else @click="cg_requireCarry(2)"></image><span>否</span>
			</p>
			<p class="floor" v-if='requireCarry==1'>
				<view v-for="(list,index) in carrylist" :key='index' @click='cg_carryindex(index)'>
					<view :class="{'active':index==carryindex}"></view>
					<view>{{list.name}}</view>
				</view>
			</p>
...
<script>
	...
	export default {
		...
		methods: {
			...
			clickStatement(type) {
				wx.navigateTo({
					url: `../statement/statement?type=${type}`
				})
			}
		}
	}
