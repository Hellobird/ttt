<template>
	<div>
		...
		<div class="mallinf2">
			<div class="mallinf2price"><span>￥{{clientGoods.makePrice}}</span><image src="../../static/close.png" v-if="iii"></image></div>
			<div>{{clientGoods.name}}</div>
			<div class="mallinf2s"><span>月销：{{clientGoods.monthSell}}笔</span><span @click='cg_pop1'>评价﹀</span><span>{{clientGoods.qualityGuaPeriod}}天无理由退换</span></div>
		</div>
		<div class="guige">
			<div class="add">
				<div class='addedit'><image src="../../static/build/minus.png"   @click.stop="minus(type)"></image><span>1</span><image src="../../static/build/add.png" @click.stop="add(type)"></image></div><span>数量</span>
			</div>
		</div>
		...
		<div class='footer'>
			<div class="edit">
				<span @click='doOrder'>立即预约</span>
				<!-- <span @click="go_build_pay">立即预约</span> -->
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			doOrder(){
				wx.setStorageSync('custominf',this.clientGoods)
